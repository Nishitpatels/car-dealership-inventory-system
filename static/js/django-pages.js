...
(function () {
  ...
  const users = () => D.users || [];
  const contactMessages = () => D.contactMessages || [];
  const stats = () => D.stats || {};
  ...
  function pageUrl(pageNumber) {
    ...
    const query = pagination.queryString ? `${pagination.queryString}&page=${pageNumber}` : `page=${pageNumber}`;
    const target = document.body.dataset.page === 'inventory' ? 'pages/inventory.html' : 'pages/search-results.html';
    return `${C.url(target)}?${query}`;
  }

  function paginationControls() {
    const pagination = D.pagination || {};
    if (!pagination.total || pagination.total <= 1) return '';
    const previous = pagination.hasPrevious ? `<li class="page-item"><a class="page-link" href="${pageUrl(pagination.previous)}"><i class="fa-solid fa-chevron-left"></i></a></li>` : `<li class="page-item disabled"><span class="page-link"><i class="fa-solid fa-chevron-left"></i></span></li>`;
    const next = pagination.hasNext ? `<li class="page-item"><a class="page-link" href="${pageUrl(pagination.next)}"><i class="fa-solid fa-chevron-right"></i></a></li>` : `<li class="page-item disabled"><span class="page-link"><i class="fa-solid fa-chevron-right"></i></span></li>`;
    const pages = pagination.pages.map(page => page === pagination.current ? `<li class="page-item active"><span class="page-link">${page}</span></li>` : `<li class="page-item"><a class="page-link" href="${pageUrl(page)}">${page}</a></li>`).join('');
    return `<nav class="mt-5" aria-label="Inventory pages"><ul class="pagination justify-content-center">${previous}${pages}${next}</ul></nav>`;
  }
  ...
  function activityList() {
    ...
    return `<div>${items.map(item => `<div class="activity-row"><span class="activity-dot"></span><div><strong class="small">${item.title}</strong><p>${item.text}</p><small>${item.date}</small></div></div>`).join('')}</div>`;
  }

  function stockHealth() {
    const s = stats();
    const total = Number(s.totalVehicles || 0);
    const healthy = Math.max(0, Number(s.availableVehicleLines || 0) - Number(s.lowStock || 0));
    const healthyPct = total ? Math.round((healthy / total) * 100) : 0;
    const lowPct = total ? Math.round((Number(s.lowStock || 0) / total) * 100) : 0;
    const outPct = Math.max(0, 100 - healthyPct - lowPct);
    return `<div class="mt-4"><div class="d-flex justify-content-between align-items-center mb-2"><strong class="small">Stock health</strong><span class="muted small">${healthyPct}% healthy</span></div><div class="progress" style="height: 9px; background: rgba(255,255,255,.08);"><div class="progress-bar bg-success" style="width:${healthyPct}%"></div><div class="progress-bar bg-warning" style="width:${lowPct}%"></div><div class="progress-bar bg-danger" style="width:${outPct}%"></div></div><div class="d-flex justify-content-between mt-2 muted small"><span>${healthy} healthy</span><span>${s.lowStock || 0} low</span><span>${s.outOfStock || 0} out</span></div></div>`;
  }

  function lowInventoryList() {
    const items = vehicles().filter(item => item.quantity <= 2).slice(0, 5);
    if (!items.length) return C.emptyState('Inventory looks healthy', 'No low-stock or out-of-stock vehicles right now.', 'fa-circle-check');
    return `<div class="admin-table-wrap"><table class="table admin-table"><thead><tr><th>Vehicle</th><th>Qty</th><th>Status</th><th class="text-end">Action</th></tr></thead><tbody>${items.map(item => `<tr><td><div class="d-flex align-items-center gap-3"><span class="table-car-thumb"><img src="${item.image}" alt="${item.make} ${item.model}"></span><div><div class="table-primary">${item.make} ${item.model}</div><div class="table-subtitle">${item.category}</div></div></div></td><td class="table-primary">${item.quantity}</td><td>${C.statusBadge(item.status)}</td><td class="text-end"><a class="form-link" href="${C.url('pages/inventory-management.html')}">Restock</a></td></tr>`).join('')}</tbody></table></div>`;
  }

  function recentUsersList() {
    const items = users().slice(0, 5);
    if (!items.length) return C.emptyState('No users found', 'Customer accounts will appear here after registration.', 'fa-users');
    return `<div>${items.map(user => `<div class="activity-row"><span class="avatar-circle">${user.initials}</span><div><strong class="small">${user.name}</strong><p>${user.role} · ${user.status}</p><small>${user.email || user.username}</small></div></div>`).join('')}</div>`;
  }
  ...
  function dashboard() {
    ...
    const alertText = [...lowStock, ...outStock].slice(0, 3).map(item => `${item.make} ${item.model}`).join(', ') || 'All inventory lines are operational';
    return `<div class="admin-content"><div class="container-fluid p-0">${C.adminPageHeader(`Good morning, ${C.displayName()}`, 'Live dealership operations, stock health, purchases and user activity from SQLite.', 'Dashboard')}<div class="row g-3 mb-4"><div class="col-6 col-xl-2">${C.statCard('fa-car', number(s.totalVehicles), 'Vehicle lines', `${number(s.availableVehicleLines)} active`)}</div><div class="col-6 col-xl-2">${C.statCard('fa-boxes-stacked', number(s.totalUnits), 'Stock units', `${money(s.inventoryValue)} value`)}</div><div class="col-6 col-xl-2">${C.statCard('fa-triangle-exclamation', number(s.criticalAlerts), 'Critical alerts', `${number(s.lowStock)} low · ${number(s.outOfStock)} out`, Number(s.criticalAlerts) > 0)}</div><div class="col-6 col-xl-2">${C.statCard('fa-receipt', number(s.totalPurchases), 'Purchases', `${money(s.averagePurchaseValue)} avg`)}</div><div class="col-6 col-xl-2">${C.statCard('fa-users', number(s.totalUsers), 'Users', `${number(s.activeUsers)} active`)}</div><div class="col-6 col-xl-2">${C.statCard('fa-envelope', number(contactMessages().length), 'Messages', 'Customer enquiries')}</div></div><div class="row g-4"><div class="col-xl-8"><div class="chart-card h-100"><div class="d-flex justify-content-between align-items-center mb-3"><div><h3 class="mb-1">Revenue overview</h3><span class="muted small">Last seven months from completed purchase records</span></div><span class="stock-badge stock-available">${money(s.revenue)}</span></div><div class="chart-holder"><canvas id="revenueChart" aria-label="Revenue chart"></canvas></div></div></div><div class="col-xl-4"><div class="chart-card h-100"><div class="d-flex justify-content-between align-items-center mb-3"><div><h3 class="mb-1">Operations command</h3><span class="muted small">Quick actions and inventory health</span></div><span class="stock-badge ${Number(s.criticalAlerts) ? 'stock-low' : 'stock-available'}">${Number(s.criticalAlerts) ? 'Needs review' : 'Healthy'}</span></div><div class="d-grid gap-2"><a class="btn-primary-gradient text-center" href="${C.url('pages/add-vehicle.html')}"><i class="fa-solid fa-plus me-1"></i>Add vehicle</a><a class="btn-outline-dealer text-center" href="${C.url('pages/inventory-management.html')}"><i class="fa-solid fa-boxes-stacked me-1"></i>Restock inventory</a><a class="btn-outline-dealer text-center" href="${C.url('pages/contact-messages.html')}"><i class="fa-solid fa-envelope me-1"></i>View messages</a><a class="btn-outline-dealer text-center" href="${C.url('pages/purchase-history.html')}"><i class="fa-solid fa-receipt me-1"></i>Review purchases</a></div>${stockHealth()}<div class="alert-card mt-4"><i class="fa-solid fa-triangle-exclamation"></i><div><strong class="small">${number(s.criticalAlerts)} inventory alerts</strong><p>${alertText}</p></div></div></div></div><div class="col-xl-7"><div class="chart-card h-100"><div class="d-flex justify-content-between align-items-center mb-3"><div><h3 class="mb-0">Inventory alerts</h3><span class="muted small">Low-stock and out-of-stock vehicles</span></div><a class="form-link" href="${C.url('pages/inventory-management.html')}">Open inventory</a></div>${lowInventoryList()}</div></div><div class="col-xl-5"><div class="chart-card h-100"><div class="d-flex justify-content-between align-items-center mb-2"><h3 class="mb-0">Recent activity</h3><span class="tiny-label">SQLite</span></div>${activityList()}</div></div><div class="col-xl-7"><div class="chart-card h-100"><div class="d-flex justify-content-between align-items-center mb-2"><h3 class="mb-0">Recent vehicles</h3><a class="form-link" href="${C.url('pages/manage-vehicles.html')}">Manage all</a></div>${recentVehicleList()}</div></div><div class="col-xl-5"><div class="chart-card h-100"><div class="d-flex justify-content-between align-items-center mb-2"><h3 class="mb-0">Recent users</h3><a class="form-link" href="${C.url('pages/user-management.html')}">Manage users</a></div>${recentUsersList()}</div></div></div></div></div>`;
  }
  ...
  function userProfile() {
    ...
    const initials = (name.split(' ').map(part => part[0]).join('').slice(0, 2) || 'NM').toUpperCase();
    return `${C.pageHero('Profile settings', `Manage your <span class="text-gradient">account.</span>`, 'Update your personal information used across the dealership purchase workflow.', [{ label: 'Home', href: 'index.html' }, { label: 'Profile' }])}<section class="section-pad"><div class="container"><div class="row g-4"><div class="col-xl-4"><div class="profile-hero"><div class="profile-avatar">${initials}</div><h2 class="fs-4 fw-bold mt-3 mb-1">${name}</h2><p class="mb-3 opacity-75 small">${auth.username || 'Customer account'}</p><span class="mini-badge bg-white text-dark">Active customer</span></div><div class="admin-form-card mt-4"><div class="tiny-label mb-3">Account overview</div><div class="d-flex justify-content-between py-2"><span class="muted small">Username</span><strong class="small">${auth.username || ''}</strong></div><div class="d-flex justify-content-between py-2"><span class="muted small">Email</span><strong class="small">${auth.email || 'Not set'}</strong></div><div class="d-flex justify-content-between py-2"><span class="muted small">Session</span><strong class="small">Active</strong></div></div></div><div class="col-xl-8"><form class="admin-form-card" method="post" action="${C.url('pages/user-profile.html')}" novalidate>${C.csrfInput()}<h2>Personal information</h2><div class="row g-3"><div class="col-md-6"><label class="form-label form-label-dealer" for="profileFirstName">First name</label><input class="form-control dealer-control" id="profileFirstName" name="first_name" value="${auth.firstName || ''}" autocomplete="given-name" required></div><div class="col-md-6"><label class="form-label form-label-dealer" for="profileLastName">Last name</label><input class="form-control dealer-control" id="profileLastName" name="last_name" value="${auth.lastName || ''}" autocomplete="family-name" required></div><div class="col-md-6"><label class="form-label form-label-dealer" for="profileUsername">Username</label><input class="form-control dealer-control" id="profileUsername" name="username" value="${auth.username || ''}" autocomplete="username" required></div><div class="col-md-6"><label class="form-label form-label-dealer" for="profileEmail">Email address</label><input class="form-control dealer-control" type="email" id="profileEmail" name="email" value="${auth.email || ''}" autocomplete="email" required></div></div><div class="form-section-rule"></div><div class="d-flex flex-wrap gap-2"><button class="btn-primary-gradient px-4 py-3" type="submit"><i class="fa-solid fa-floppy-disk me-1"></i>Save changes</button><a class="btn-outline-dealer px-4 py-3 text-center" href="${C.url('pages/user-dashboard.html')}">Back to dashboard</a></div></form></div></div></div></section>`;
  }

  function adminProfile() {
    const auth = C.authState();
    const name = C.displayName();
    const initials = (name.split(' ').map(part => part[0]).join('').slice(0, 2) || 'AM').toUpperCase();
    return `<div class="admin-content"><div class="container-fluid p-0">${C.adminPageHeader('Your profile', 'Manage the information displayed across your dealership workspace.', 'Profile')}<div class="row g-4"><div class="col-xl-4"><div class="profile-hero"><div class="profile-avatar">${initials}</div><h2 class="fs-4 fw-bold mt-3 mb-1">${name}</h2><p class="mb-3 opacity-75 small">Dealership administrator</p><span class="mini-badge bg-white text-dark">Nexus Motors HQ</span></div><div class="admin-form-card mt-4"><div class="tiny-label mb-3">Account overview</div><div class="d-flex justify-content-between py-2"><span class="muted small">Username</span><strong class="small">${auth.username || 'admin'}</strong></div><div class="d-flex justify-content-between py-2"><span class="muted small">Session</span><strong class="small">Active</strong></div><div class="d-flex justify-content-between py-2"><span class="muted small">Workspace role</span><strong class="small">Superuser</strong></div></div></div><div class="col-xl-8"><form class="admin-form-card" method="post" action="${C.url('pages/profile.html')}" novalidate>${C.csrfInput()}<h2>Profile information</h2><div class="row g-3"><div class="col-md-6"><label class="form-label form-label-dealer" for="adminProfileFirstName">First name</label><input class="form-control dealer-control" id="adminProfileFirstName" name="first_name" value="${auth.firstName || ''}" autocomplete="given-name" required></div><div class="col-md-6"><label class="form-label form-label-dealer" for="adminProfileLastName">Last name</label><input class="form-control dealer-control" id="adminProfileLastName" name="last_name" value="${auth.lastName || ''}" autocomplete="family-name" required></div><div class="col-md-6"><label class="form-label form-label-dealer" for="adminProfileEmail">Email address</label><input class="form-control dealer-control" type="email" id="adminProfileEmail" name="email" value="${auth.email || ''}" autocomplete="email" required></div><div class="col-md-6"><label class="form-label form-label-dealer" for="adminProfileUsername">Username</label><input class="form-control dealer-control" id="adminProfileUsername" value="${auth.username || ''}" readonly></div></div><div class="form-section-rule"></div><button class="btn-primary-gradient px-4 py-3" type="submit">Save profile changes</button></form></div></div></div></div>`;
  }

  function contact() {
    return `${C.pageHero('Contact us', 'Let us find the <span class="text-gradient">right drive.</span>', 'Speak with the Nexus Motors team about inventory, a test drive or your next dealership visit.', [{ label: 'Home', href: 'index.html' }, { label: 'Contact' }])}<section class="section-pad"><div class="container"><div class="row g-4"><div class="col-lg-5"><div class="surface-card p-4 h-100"><div class="page-kicker">Visit or call</div><h2 class="fs-3 fw-bold mt-2">We are here to help.</h2><div class="contact-item"><div class="info-icon"><i class="fa-solid fa-phone"></i></div><div><strong>Call the showroom</strong><span>+1 800 654 0321 · Mon-Sat, 08:00-19:00</span></div></div><div class="contact-item"><div class="info-icon"><i class="fa-solid fa-envelope"></i></div><div><strong>Email our team</strong><span>hello@nexusmotors.test</span></div></div><div class="contact-item"><div class="info-icon"><i class="fa-solid fa-location-dot"></i></div><div><strong>Visit the showroom</strong><span>240 Horizon Drive, North District, NY 10001</span></div></div></div></div><div class="col-lg-7"><form class="surface-card p-4 p-md-5" method="post" action="${C.url('pages/contact.html')}" novalidate>${C.csrfInput()}<div class="page-kicker">Send a message</div><h2 class="fs-3 fw-bold mt-2 mb-4">How can we help?</h2><div class="row g-3"><div class="col-md-6"><label class="form-label form-label-dealer" for="contactName">Your name</label><input class="form-control dealer-control" id="contactName" name="name" required placeholder="Alex Morgan"></div><div class="col-md-6"><label class="form-label form-label-dealer" for="contactEmail">Email</label><input class="form-control dealer-control" id="contactEmail" type="email" name="email" required placeholder="alex@example.com"></div><div class="col-12"><label class="form-label form-label-dealer" for="contactSubject">I am interested in</label><select class="form-select dealer-select" id="contactSubject" name="subject" required><option>Buying a vehicle</option><option>Booking a test drive</option><option>Vehicle availability</option><option>Dealership support</option></select></div><div class="col-12"><label class="form-label form-label-dealer" for="contactMessage">Message</label><textarea class="form-control dealer-control" id="contactMessage" name="message" rows="5" required placeholder="Tell us about the vehicle or service you need..."></textarea></div><div class="col-12"><button class="btn-primary-gradient px-4 py-3" type="submit">Send message <i class="fa-solid fa-paper-plane ms-1"></i></button></div></div></form></div><div class="col-12"><div class="map-panel"><span class="map-pin"><i class="fa-solid fa-location-dot"></i></span></div></div></div></div></section>`;
  }
  ...
  function searchResults() {
    ...
    return `${C.pageHero('Search results', `Search results for <span class="text-gradient">${queryLabel}</span>`, 'Browse matching inventory, then fine-tune the list with the filters.', [{ label: 'Home', href: 'index.html' }, { label: 'Inventory', href: 'pages/inventory.html' }, { label: 'Search results' }])}<section class="section-pad"><div class="container"><div class="row g-4"><div class="col-lg-3">${C.filterPanel()}</div><div class="col-lg-9"><div class="surface-card p-3 mb-4 d-flex gap-3 align-items-center"><div class="info-icon flex-shrink-0"><i class="fa-solid fa-magnifying-glass"></i></div><div><strong>${vehicles().length} close matches found</strong><p class="muted small mb-0">Results are drawn from the live SQLite vehicle inventory.</p></div></div><div class="results-toolbar d-flex justify-content-between align-items-center mb-4"><span class="results-count" id="resultsCount">Showing ${vehicles().length} matching vehicles</span></div><div class="row g-4" id="vehicleResults">${vehicles().map(item => `<div class="col-md-6 col-xl-4">${C.vehicleCard(item)}</div>`).join('') || `<div class="col-12">${C.emptyState('No Vehicles Found', 'Try changing your filters or clearing the current search.', 'fa-magnifying-glass')}</div>`}</div>${paginationControls()}</div></div></div></section>`;
  }

  function inventoryPage() {
    return `${C.pageHero('Vehicle inventory', 'Find your <span class="text-gradient">next drive.</span>', 'Explore live dealership inventory. Use filters to narrow down the right match.', [{ label: 'Home', href: 'index.html' }, { label: 'Inventory' }])}<section class="section-pad"><div class="container"><div class="row g-4"><div class="col-lg-3">${C.filterPanel()}</div><div class="col-lg-9"><div class="results-toolbar d-flex flex-wrap align-items-center justify-content-between gap-3"><div><div class="results-count" id="resultsCount">Showing ${vehicles().length} vehicles</div><div class="d-flex gap-2 flex-wrap mt-2" id="activeFilters"></div></div></div><div class="row g-4" id="vehicleResults">${vehicles().map(item => `<div class="col-md-6 col-xl-4">${C.vehicleCard(item)}</div>`).join('') || `<div class="col-12">${C.emptyState('No Vehicles Found', 'Try changing your filters or clearing the current search.', 'fa-magnifying-glass')}</div>`}</div>${paginationControls()}</div></div></div></section>`;
  }

  function contactMessagesPage() {
    const rows = contactMessages().map(item => `<tr><td><div class="table-primary">${item.name}</div><div class="table-subtitle">${item.email}</div></td><td>${item.subject}</td><td><div class="table-subtitle">${item.message}</div></td><td>${item.submittedAt}</td><td class="text-end"><form class="d-inline" method="post" action="${C.url('actions/delete-contact-message.html')}" onsubmit="return confirm('Delete this contact message?');">${C.csrfInput()}<input type="hidden" name="message_id" value="${item.id}"><button class="action-icon delete ms-1" type="submit" aria-label="Delete message from ${item.name}"><i class="fa-solid fa-trash"></i></button></form></td></tr>`).join('');
    return `<div class="admin-content"><div class="container-fluid p-0">${C.adminPageHeader('Contact messages', 'Read customer enquiries submitted from the public contact page.', 'Messages')}<div class="admin-table-wrap"><table class="table admin-table"><thead><tr><th>Sender</th><th>Subject</th><th>Message</th><th>Submitted</th><th class="text-end">Actions</th></tr></thead><tbody>${rows || emptyRow('No contact messages found', 5, 'fa-envelope')}</tbody></table></div></div></div>`;
  }
  ...
  function errorPage(code, title, text, icon) {
    return `${C.pageHero(`Error ${code}`, `${title} <span class="text-gradient">${code}</span>`, text, [{ label: 'Home', href: 'index.html' }, { label: `Error ${code}` }])}<section class="section-pad"><div class="container"><div class="surface-card p-4 p-md-5 text-center"><div class="empty-state-icon mx-auto mb-3"><i class="fa-solid ${icon}"></i></div><h2 class="fs-3 fw-bold mb-3">${title}</h2><p class="muted mx-auto copy-narrow mb-4">${text}</p><div class="d-flex justify-content-center gap-2 flex-wrap"><a class="btn-primary-gradient px-4 py-3" href="${C.url('index.html')}">Back home</a><a class="btn-outline-dealer px-4 py-3" href="${C.url('pages/inventory.html')}">Browse inventory</a></div></div></div></section>`;
  }
  ...
  const overrides = {
    ...
    '403': () => errorPage('403', 'Access denied', 'You do not have permission to open this dealership workspace page.', 'fa-lock'),
    '500': () => errorPage('500', 'Something went wrong', 'The dealership workspace hit an unexpected error. Please try again shortly.', 'fa-triangle-exclamation'),
