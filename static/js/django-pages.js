...
(function () {
  ...
  function activityList() {
    ...
    if (!items.length) return C.emptyState('No recent purchases', 'Completed purchases will appear here automatically.', 'fa-receipt');
    return `<div class="activity-list">${items.map(item => `<div class="activity-item"><span class="activity-dot"></span><div><strong>${item.customer}</strong><p>Purchased ${item.quantity} × ${item.vehicle} for ${item.total}</p><small>${item.date}</small></div></div>`).join('')}</div>`;
  }
